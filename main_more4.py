...
from dataloader import RecData, UserItemData, RecData2
import argparse
import numpy as np
    ...
    for epoch in range(config.epoch):
        ...
        for i in range(num_batch):
            ...
            if sampler.__class__.__name__ in ["two_pass_rank", "two_pass_weight_rank"]:
                ...
                loss = model.loss_function(neg_rat, prob_neg, pos_rat, user_id=user_id, pos_rank=pos_rank.to(device), reduction=config.reduction, weighted=config.weighted, lambda_w = config.lambda_w, loss_type = config.loss_type, beta=config.beta)
            else:
                loss = model.loss_function(neg_rat, prob_neg, pos_rat, user_id=user_id, reduction=config.reduction, weighted=config.weighted, lambda_w = config.lambda_w, loss_type = config.loss_type, beta=config.beta)
...
def main(config, logger=None):
    device = torch.device(config.device)
    data = RecData2(config.data_dir, config.data)
