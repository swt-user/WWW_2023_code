...
from dataloader import RecData, UserItemData, data_prefetcher
import argparse
import numpy as np
...
def train_model(model, sampler, train_mat, test_mat, config, logger):
    ...
    for epoch in range(config.epoch):
        ...
        prefetcher = data_prefetcher(random_idx, train_data, config.batch_size, config.device)
        user_id, item_id = prefetcher.next()
